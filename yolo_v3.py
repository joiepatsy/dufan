...
def batch_norm(inputs, training, data_format):
    return tf.keras.layers.BatchNormalization(axis=1 if data_format == 'channels_first' else 3, momentum=_BATCH_NORM_DECAY, epsilon=_BATCH_NORM_EPSILON, scale=True, trainable=training)(inputs)
...
def conv2d_fixed_padding(inputs, filters, kernel_size, data_format, strides=1):
    ...
    return tf.keras.layers.Conv2D( filters=filters, kernel_size=kernel_size, strides=strides, padding=('SAME' if strides == 1 else 'VALID'), use_bias=False, data_format=data_format)(inputs)
