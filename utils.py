...
def draw_frame(frame, frame_size, boxes_dicts, class_names, model_size):
    ...
    for cls in range(len(class_names)):
        boxes = boxes_dict[cls]
        if(class_names[cls] == 'bus'):
            bus += len(boxes)
            extdrawframe(frame, frame_size, boxes_dicts, class_names, model_size, cls, resize_factor, colors, boxes)
        if(class_names[cls] == 'car'):
            car += len(boxes)
            extdrawframe(frame, frame_size, boxes_dicts, class_names, model_size, cls, resize_factor, colors, boxes)
        if(class_names[cls] == 'truck'):      
            truck += len(boxes)
            extdrawframe(frame, frame_size, boxes_dicts, class_names, model_size, cls, resize_factor, colors, boxes)
        if(class_names[cls] == 'motorbike'):      
            motor += len(boxes)  
            extdrawframe(frame, frame_size, boxes_dicts, class_names, model_size, cls, resize_factor, colors, boxes)
    data = [bus, car, truck, motor]
    return data

def extdrawframe(frame, frame_size, boxes_dicts, class_names, model_size, cls, resize_factor, colors, boxes):
    color = colors[cls]
    ...
    if np.size(boxes) != 0:
        for box in boxes:
            ...
            (test_width, text_height), baseline = cv2.getTextSize(class_names[cls], cv2.FONT_HERSHEY_SIMPLEX, 0.75, 1)
            cv2.rectangle(frame, (xy[0], xy[1]), (xy[0] + test_width, xy[1] - text_height - baseline), color[::-1], thickness=cv2.FILLED)
